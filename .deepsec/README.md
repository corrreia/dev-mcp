...
Currently configured project: `dev-mcp` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/dev-mcp/SETUP.md` to fill in
   `data/dev-mcp/INFO.md`.
...
`--project-id dev-mcp` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/dev-mcp/`.
