# Agent setup for `dev-mcp`

This is a deepsec scanning workspace. Project `dev-mcp` was just registered
(target: `..`). Setup is incomplete — `data/dev-mcp/INFO.md`
...
2. **Fill in `data/dev-mcp/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id dev-mcp
pnpm deepsec process --project-id dev-mcp
```
